...
int get_archive_file_list(const char *archive_name, file_list_t *files) {
    ...
    FILE * fr = fopen(archive_name, "rb");
    if (fr == NULL) {
        return -1;
    }

    long int end;
    fseek(fr, -1 * BLOCK_SIZE * NUM_TRAILING_BLOCKS, SEEK_END);
    end = ftell(fr);
    fseek(fr, 0, SEEK_SET);
    ...
    do {
        ...
        sscanf(header.size, "%o", &file_bytes);
        count = (file_bytes + BLOCK_SIZE - 1) / BLOCK_SIZE;
        file_bytes += ((BLOCK_SIZE * count) - file_bytes);
        fseek(fr, file_bytes, SEEK_CUR);
    } while ((nbytes = ftell(fr)) < end);
...
//         sscanf(header.size, "%o", &file_bytes);
//         printf("%s\n", header.name);
//     } while ((nbytes = fseek(fr, file_bytes, SEEK_CUR)) > 0);